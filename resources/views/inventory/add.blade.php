        ...
        <form method="POST" action="{{route('inventory.store')}}">
            ...
            <div class="card-body">
                <div class="form-group row">
                    ...
                    <div class="col-sm-6 mb-3 mt-3 mb-sm-0">
                        ...
                        <select name="stock_id" class="form-control @error('stock') is-invalid @enderror">
                            @foreach($stocks as $stock)
                                <option value="{{ $stock['id'] }}" @if (old('stock_id') == $stock['id']) selected @endif>
                                    {{ $stock['name'] }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-sm-6 mb-3 mt-3 mb-sm-0">
                        <span style="color:red;">*</span> Accepted

                        <select name="user_created" class="form-control @error('user_created') is-invalid @enderror">
                            @foreach($users as $user)
                                <option value="{{ $user['id'] }}" @if (old('user_created') == $user['id']) selected @endif>
                                    {{ $user['full_name'] }}
                                </option>
                            @endforeach
                        </select>
